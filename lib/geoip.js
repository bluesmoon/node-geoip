...
function regionName(countryCode, regionCode){
	return regions[countryCode] ? regions[countryCode][regionCode] : null;
}

function loadRegions(callback){
	var file = path.join(geodatadir, 'geoip-region-names.json');
	...
	if (typeof arguments[0] === 'function') {
		...
		regions = JSON.parse(fs.readFileSync(file, 'utf8'));
	}
}
...
module.exports = {
	...
	regions: function(countryCode){
		return (countryCode ? regions[countryCode] : regions) || []; 
	},

	regionName: function(countryCode, regionCode){
		if(!countryCode || !regionCode) {
			return null;
		} else if (typeof countryCode === 'string') {
			var rCode = regionCode;

			if(typeof rCode === 'number'){
				rCode = rCode + "";
			...
			if(typeof countryCode === 'string' && typeof rCode === 'string'){
				return regionName(countryCode, rCode);
			}
		}
